 ...
 * Brugge Framework.
 */
package io.codeffeine.brugge.usecase.migration;
...
import io.codeffeine.brugge.usecase.sort.topological.DirectedGraph;
import io.codeffeine.brugge.usecase.sort.topological.TolopogicalSort;
...
public class PersistMigrationData implements PersistMigrationDataInterface {
    ...
    @Override
    public void persist() {
        Reflections reflections = new Reflections("io.codeffeine.brugge.domain.migration.data");
        ...
        for (InitData initData : InitData.getInitData()) {
            ...
            for (Field f : initData.getTargetClass().getDeclaredFields()) {
                if (f.getType().getPackage() != null && f.getType().getPackage().getName().contains("io.codeffeine.brugge.domain")) {
